...
/* ---------------------------
   Quotes System (Full-Screen Marquee)
   Fetches quotes automatically
   Smooth marquee animation + hover pause
   --------------------------- */
async function initQuotes() {
  const wrapper = document.createElement("div");
  wrapper.id = "quoteWrapper";
  ...
  const jsonPath = window.config?.quotesPath || "quotes.json";
  let quotes = [];
  ...
  // Drag-to-pause
  wrapper.addEventListener("mousedown", () => { paused = true; quoteBox.style.cursor = "grabbing"; });
  window.addEventListener("mouseup", () => { paused = false; quoteBox.style.cursor = "grab"; });
